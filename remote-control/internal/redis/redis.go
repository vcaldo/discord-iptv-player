...
// SearchChannelsByNameUnlimited works like SearchChannelsByName but without the 25-result limit
// It's intended for the search command rather than autocomplete
func (c *Client) SearchChannelsByNameUnlimited(guildID, playlistName, searchTerm string) ([]models.TvChannel, error) {
	var channels []models.TvChannel

	err := c.instrumentOperation("search-channels-by-name-unlimited", func() error {
		playlistKey := fmt.Sprintf("guild:%s:playlist:%s", guildID, playlistName)
		channelsKey := fmt.Sprintf("%s:channels", playlistKey)
		indexName := fmt.Sprintf("idx:%s:%s:channels", guildID, playlistName)
		...
		// We set a high limit (10000) to effectively get all results
		result, err := c.rdb.Do(
			"FT.SEARCH", indexName,
			query,
			"LIMIT", "0", "10000",
		...
		log.Printf("found %d channels matching search term '%s' in playlist '%s' for guild %s (unlimited results)",
			len(channels), searchTerm, playlistName, guildID)
		return nil
	})
